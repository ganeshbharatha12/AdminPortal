...
export class EmployeePortalComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.brokerregform = this.formBuilder.group({
      ...
      certificateName: [''],
      Insuranceexpiry: [''],
      stc_generation:['', [Validators.required]],
      user_generated_stc:['', [Validators.required]],
      sales_tracking_code:['', [Validators.required]],
  ...
  editadvisorformDetails(e,i,form){
    ...
    if(form.planLevels && form.planLevels.length){
...
//  }
  ...
  public checkSalesTrackingCodeAuto(event) {
    if (event.target.value == 'false') {
      ...
      this.broker['sales_tracking_code'].setValidators([Validators.required]);
      this.broker['sales_tracking_code'].updateValueAndValidity();
    }
  }
  ...
  // }
    ...
    if (this.brokerregform.invalid) {
     ...
     console.log(invalid)
      ...
      let obj = {
        EOInsurense: this.capitalize(this.brokerregform.value.InsuranceName),
        EOCertificate: this.brokerregform.value.certificateName,
        policy: this.brokerregform.value.policyNumber,
        EOIexpiryDate:
          this.datePipe.transform(
            this.brokerregform.value.Insuranceexpiry,
            'yyyy-MM-dd'
          ) || '',
      };
...
let FormDetails:any = this.addformDetails
    ...
    for(let i=0;i<FormDetails.length;i++){
      ...
      formdata.append('sales_tracking_code', this.brokerregform.value.sales_tracking_code || "");
      formdata.append(
        'secondary_phone',
      ...
      formdata.append('licenses', sessionStorage.getItem('addlicenseformData'));
  ...
  viewfulldetailsofBroker(id){
    ...
    this.router.navigate(['/editAdvisor/'+this.finalSummaryBrookerDataid]);
    // alert(id)
  }
  closesummarydetails(){
