...
// import { PhoneNoFormatPipe } from '../pipes/phone-no-format.pipe';
...
public show:boolean = false;
  ...
  constructor(
    ...
    // private phoneNoFormat: PhoneNoFormatPipe,
...
// video.play();
    ...
    this.paymentUri = this.url+"employee/5334/details";;

    this.paymentsuccessuri = this.url+"employee/5334/details";
    ...
    this.addressform = this.formBuilder.group({
      ...
      secondaryPhone: ['', [Validators.minLength(14), Validators.maxLength(14)]],
    }, { validators: [phoneNumbersMustBeDifferent]});
...

{
  ...
  getCustomerData() {
    ...
    this.http.get(environment.apiUrl+'/api/ap/admin'+customerDataEndpoint, {
      ...
      if(response.status==200) {
        ...
        this.roeUrl = this.customerData.ROE;
        ...
        this.fusebillId = parseInt(this.customerData.fusebillId);
        ...
        this.secondaryPhone = this.customerData.secondaryPhone;
        ...
        this.customerData.secondaryPhone = this.customerData.secondaryPhone
  ...
  updateDependent() {
    if(this.dependentform.invalid) {
      ...
      if(data["graduationDate"]){
        data["graduationDate"]= this.datePipe.transform(new Date(this.dependentform.value.graduationDate).toLocaleString(),'MM-dd-yyyy')
      ...
      if (result.isConfirmed) {
        ...
        if(this.hicList.length>0) {
          for(let i=0;i<this.hicList.length;i++) {
              this.http.get(this.hicList[i].healthCard, { responseType: "arraybuffer" }).subscribe(
              pdf => {
  ...
  downloadHic(value:any) {
    this.http.get(value.healthCard, { responseType: "arraybuffer" }).subscribe(
      pdf => {
        const blob = new Blob([pdf], { type: "application/pdf" });
  ...
  updateCorporatePAD() {
    ...
    formData.append("session",this.encBank) || "";
    formData.append("timestamp", new Date().getTime());
    let Endpoint = '/customer/'+this.customerid+'/generatePAD';
    // let data = this.paymentMethodForm.value.files;
    this.http.post(environment.apiUrl+'/api/ap/admin'+Endpoint, formData, {
      headers:{
        Authorization: 'Bearer ' + this.accessToken,
    }}).subscribe((response:any)=>{
      if(response.status==200) {
  ...
  addDependentData() {
    if(this.dependentform.valid) {
      ...
      if(this.dependentData['graduationDate']){
        this.dependentData['graduationDate'] = this.datePipe.transform(new Date(this.dependentform.value.graduationDate).toLocaleString(),'MM-dd-yyyy')
